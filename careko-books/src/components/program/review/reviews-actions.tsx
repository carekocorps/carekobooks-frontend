      ...
      if (existingReview) {
        ...
        try {
          ...
          if (error.response?.status === 409) {
            const review = await ReviewService.getReviewByUserAndBook(user.username, bookId);
            if (review) {
              result = await ReviewService.updateReview(review.id, submissionData as UpdateBookReview);
              toast.success("Revisão atualizada com sucesso após conflito!");
            } else {
              throw error;
            }
          } else {
